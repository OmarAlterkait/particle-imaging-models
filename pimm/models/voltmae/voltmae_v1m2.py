...
def _scatter_mean(src: torch.Tensor, index: torch.Tensor, num_groups: int):
    """Mean-pool (N, C) → (num_groups, C) defined by index (N,)."""
    out = src.new_zeros(num_groups, src.shape[1])
    count = src.new_zeros(num_groups, 1)
...
class PatchLocalMixer(nn.Module):
    """PointNet-style intra-patch feature mixing (no cross-patch information flow).

    Each layer: pre-norm → concat(point, patch_mean) → project → residual add.
    Stacking layers lets the patch mean progressively refine.
    """

    def __init__(self, in_channels: int, channels: int, num_layers: int = 2,
                 post_norm: bool = True):
        super().__init__()
        self.proj_in = nn.Sequential(nn.Linear(in_channels, channels), nn.GELU())
        ...
        self.post_norm = nn.LayerNorm(channels) if post_norm else nn.Identity()

    def forward(self, x: torch.Tensor, patch_ids: torch.Tensor, num_patches: int):
        x = self.proj_in(x)
        for layer in self.layers:
            h = layer["norm"](x)
            pool = _scatter_mean(h, patch_ids, num_patches)
            x = x + layer["mlp"](torch.cat([h, pool[patch_ids]], dim=-1))
...
class VoltMAE(PointModel):
    ...
    def __init__(
        ...
        # patch-local point mixer: intra-patch mean-pool-and-broadcast with
        # residual MLPs — mixes point features within each stride^3 patch
        # without any cross-patch information flow.
        self.stem = PatchLocalMixer(
            in_channels, stem_channels,
            num_layers=stem_layers, post_norm=stem_post_norm,
        )
    ...
    def _compute_cu_seqlens(batch_indices: torch.Tensor):
        ...
        cu = torch.zeros(counts.numel() + 1, dtype=torch.int32, device=batch_indices.device)
        cu[1:] = torch.cumsum(counts.to(torch.int32), dim=0)
        max_seqlen = int(counts.max().item()) if counts.numel() else 0
        return cu, max_seqlen

    @staticmethod
    def _compute_patch_ids(grid_coord, batch, stride):
        """Assign each point a compact patch id from (batch, grid_coord // stride)."""
        pc = grid_coord // stride
        dims = pc.max(0).values + 1
        flat = (batch * (dims[0] * dims[1] * dims[2])
                + pc[:, 0] * (dims[1] * dims[2])
                + pc[:, 1] * dims[2]
                + pc[:, 2])
        _, ids = flat.unique(return_inverse=True)
        return ids, int(ids.max().item()) + 1
    ...
    def forward(self, data_dict):
        ...
        # 1. Patch-local point mixing then sparse tokenization.
        #    The +96 buffer mirrors the Volt reference implementation
        #    (libs/Volt/pointcept/models/volt/volt_base.py:240).
        sparse_shape = torch.add(torch.max(grid_coord, dim=0).values, 96).tolist()
        patch_ids, num_patches = self._compute_patch_ids(
            grid_coord, batch, self.stride
        )
        mixed = self.stem(feat, patch_ids, num_patches)
    ...
    def encode(self, data_dict):
        ...
        patch_feat = tokens.index_select(0, p2t)  # (N, embed_dim)
        return torch.cat([patch_feat, mixed], dim=-1)  # (N, embed_dim + stem_channels)
